...
function websocket_add_listener(client_sock) {
    ...
    client_sock.on("message", function (data) {
        ...
        var reg = RegExp(/"clientSubscribeData","chemicalexperimentations"/);
        if (data.match(reg)) {
            ...
            getItem(mzkqUid).then(data1=>{
                ...
                if (data.match(/"id":/)) {
                    ...
                    client_sock.send('43/sync-data/chemicalexperimentations,' + s + '[null,' + JSON.stringify(ulist) + ']');
        ...
        if (data.match(regce)) {
            var regcereg = RegExp(/"chemicalexperimentations"/);
            ...
            if (data.match(regcereg)) {
                if (data.match(regcebase)) {
                    if (data.match(/"obj"/)) {
                        ...
                        client_sock.send('43/sync-data/chemicalexperimentations,' + s + '[null,{"version":' + did + '}]');
                    }
                }
            }
        }
        ...
        if (data.match(regce2)) {
            ...
            if (data.match(regcereg2)) {
                ...
                if (data.match(/"chemicalexperimentations"/)) {
        ...
        var regbyid = RegExp(/"clientRequestFullDataById","chemicalexperimentations"/);
        if (data.match(regbyid)) {
            ...
            getItem(jsonarrs[2]).then(data=>{
                warejson =data
                client_sock.send('43/sync-data/chemicalexperimentations,' + s + '[null,' + warejson + ']');
            }).catch(e=>console.log(e));
        }
        ...
        if (data == '40/sync-data/chemicalexperimentations?uid=4516289&companyId=&token=xZDUdKrdmXeePP8cUMAbnECTYKV6JH8aYZlIIRu0,') {
              console.log("自动登录")
            mzkqUid = 4516289;
            client_sock.send('40');
            client_sock.send('40/sync-data/chemicalexperimentations,');
        }
...
function on_server_client_comming(client_sock) {
    ...
    client_sock.send('0{"sid":"_jR3fuMp9IHMfcq-Bi_u","upgrades":[],"pingInterval":25000,"pingTimeout":5000}');
